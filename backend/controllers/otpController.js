...
return res.json({
                token,
                user: { id: newUser.insertId, name: "", phone: cleanPhone, role: "customer" }
              });
            }
          );
        }
...
//   // Check for test numbers / dev bypass (Numbers starting with 99999 or 9876543210)
//   const isTestNumber = cleanPhone.startsWith("99999") || cleanPhone === "9876543210";
...
//       if (result.length > 0 && result[0].attempts >= 5 && !isTestNumber) {
//         return res.status(429).json({ message: "Daily OTP limit reached. Try again tomorrow." });
//       }

//       // Hardcode 123456 for test numbers, otherwise generate random OTP
...
//       const params = [otp, expiry, cleanPhone];

//       db.query(saveOtpQuery, params, (err) => {
//         if (err) return res.status(500).json({ message: err.message });

//         // Skip SMS API call for local testing numbers
//         if (isTestNumber) {
//           return res.json({ message: "Test OTP Sent Successfully (Use 123456)" });
//         }

//         // Send SMS via Fast2SMS for real numbers
//         axios.post(
//           "https://www.fast2sms.com/dev/bulkV2",
//           {
//             route: "q",
...
//   const isTestNumber = cleanPhone.startsWith("99999") || cleanPhone === "9876543210";
//   const isDevBypass = isTestNumber && otp === "123456";

//   const handleUserLoginOrCreate = () => {
//     // Cleanup verified OTP
//     db.query(`DELETE FROM otp_verifications WHERE phone=?`, [cleanPhone]);
...
//         // Register new user with empty name so Monkey Popup triggers
//         db.query(
//           `INSERT INTO users (name, phone, password, role) VALUES (?, ?, ?, ?)`,
//           ["", cleanPhone, "OTP_LOGIN", "customer"],
//           (err, newUser) => {
...
//                 return res.json({
//                   token,
//                   user: { id: newUserId, name: "", phone: cleanPhone, role: "customer" }
//                 });
//               }
//             );
//           }
...
//   // Skip DB OTP verification check if test credentials are used
//   if (isDevBypass) {
//     return handleUserLoginOrCreate();
//   }
